...
| **`MADV_DONTNEED` / `MADV_FREE` on idle pages** (not just whole segments) | Backend method `page_reset(ptr, size) -> bool` implemented (Linux `MADV_DONTNEED`, macOS/FreeBSD `MADV_FREE`, Windows `VirtualAlloc(MEM_RESET)`); arena-level wire-through still pending | jemalloc, tcmalloc, mimalloc page reset | Backend seam implemented. Wire-through to `mnemosyne-arena` purge for partial idle-page reset is the remaining work. | partial |
...
| --- | --- | --- | --- |
| ~~Sprint A~~ **partially delivered** | `MemoryBackend::page_reset(ptr, size) -> bool` added with default `false` impl; unix backend uses `MADV_DONTNEED` (Linux) / `MADV_FREE` (macOS, FreeBSD); windows backend uses `VirtualAlloc(MEM_RESET)`; wrapper records `page_reset_calls` / `page_reset_bytes` telemetry without touching `current_mapped_bytes`. Wire-through to `mnemosyne-arena` for partial-segment idle-page reset remains a follow-on. | `[minor]` | `page_reset_telemetry_increments_call_and_byte_counters_only`, `wrapper_page_reset_round_trips_on_active_mapping`, `wrapper_page_reset_rejects_null_and_zero` |
