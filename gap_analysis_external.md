...
| Huge-page request flags (`MADV_HUGEPAGE`, `MAP_HUGETLB`, `MEM_LARGE_PAGES`) | Not requested | jemalloc THP, mimalloc large page, tcmalloc HugePageHeap | 2 MiB segments are already large-page-aligned; setting the hint reduces TLB pressure for large workloads. | `[minor]` |
...
| Sprint A | Add `madvise(MADV_HUGEPAGE)` / `MAP_HUGETLB` hints in `UnixBackend::allocate` when `size >= SEGMENT_SIZE`. | `[patch]` | `large_allocation_requests_hugepage_hint` |
