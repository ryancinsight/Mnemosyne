# Summary

[Introduction](README.md)

# Part I — Allocation Architecture

- [1. Size Classes](size_classes.md)
  - [Example: Size Classes](examples/size_classes.md)
- [2. Allocation Policies](alloc_policies.md)
  - [Example: Allocator Statistics](examples/alloc_policies.md)
- [3. The Global Allocator](global_allocator.md)

# Part II — Memory Management

- [4. Scratch Pools](scratch_pools.md)
  - [Example: Scratch Pool](examples/scratch_pool.md)
- [5. Segment Lifecycle](segment_lifecycle.md)
- [6. Decay, Purge, and Reset](decay_purge_reset.md)

# Part III — Placement and Hardening

- [7. NUMA-Aware Allocation](numa_placement.md)
- [8. Hardened and Secure Policies](hardened_secure.md)
- [9. Profiling and Leak Detection](profiling.md)

# Part IV — The Atlas Stack

- [10. Position in the Stack](stack_position.md)
