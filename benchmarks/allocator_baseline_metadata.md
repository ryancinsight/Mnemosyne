...
The current usable-size comparison measured Mnemosyne at `6.869 ns` for 32-byte cycles and `6.862 ns` for 1024-byte cycles on this Windows GNU target.
The current realloc comparison measured Mnemosyne at `7.017 ns` for within-class `24 -> 32` cycles and `15.774 ns` for cross-class `32 -> 64` cycles on this Windows GNU target.
...
The current selected mimalloc-regression refresh measured Mnemosyne at `17.373 us` for threaded small allocation cycles, `76.284 us` for threaded saturated small allocation cycles, `6.869 ns` for `usable size latency/small_32`, and `7.017 ns` for `realloc latency/within_class_24_to_32`. The refreshed variance report marks these Mnemosyne rows stable under their row-specific CI-width thresholds.
