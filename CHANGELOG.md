...
- Completed the per-component complexity review in `docs/complexity_audit.md`: added C-ABI-shim and backend operation tables (all entry points O(1) plus the O(n) inherent work of `calloc` zeroing and cross-class `realloc` copy) and recorded the landed `Page::index_in_segment()` O(1) derivation foundation.
