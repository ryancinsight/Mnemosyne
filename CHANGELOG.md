...
- Added `SEGMENT_TAIL_GUARD_SIZE = 4 KiB` constant (with compile-time `is_power_of_two` and `<= SEGMENT_ALIGN` checks) and made `allocate_segment` install a `PROT_NONE` / `PAGE_NOACCESS` guard at `aligned_addr + SEGMENT_SIZE` on every fresh OS-backed segment. The guard lives in the alignment slack the arena already reserves and is released along with the rest of the segment by `B::deallocate(raw_ptr, SEGMENT_MAPPING_SIZE)`. Forward OOB writes that walk past Page 31 now trap inside the segment mapping instead of corrupting an unrelated mapping.
