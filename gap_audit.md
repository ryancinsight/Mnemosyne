...
- [resolved] Sprint A from `gap_analysis_external.md` added the
  `MemoryBackend::page_reset(ptr, size) -> bool` seam and wired it through the
  arena's `reset_segment_pool` path. `segment/alloc.rs` resets each retained
  segment's user pages (`segment + PAGE_SIZE`, `SEGMENT_SIZE - PAGE_SIZE`),
  preserves the page-0 header, and re-caches the segment. The provider backend
  and arena contracts are pinned by `wrapper_page_reset_round_trips_on_active_mapping`,
  `wrapper_page_reset_rejects_null_and_zero`, and
  `test_reset_segment_pool_propagates_correct_bounds`; telemetry remains
  separate from mapped-byte accounting. No arena-level page-reset follow-on
  remains in this scope.
- [resolved-history] The original Sprint A implementation note described the
  backend seam as lacking an arena consumer. It is superseded by the resolved
  entry above: `reset_segment_pool`, `mnemosyne::reset`, and their telemetry are
  now the canonical arena consumer and are covered by the named tests.
- [resolved] Sprint B from `gap_analysis_external.md` added the
  `MemoryBackend::make_guard(ptr, size) -> bool` seam and wired it through
  arena segment creation. With the opt-in `segment-header-guards` feature,
  `segment/alloc.rs` protects the end of Page 0; with
  `segment-tail-guards`, it protects the alignment slack immediately after
  `SEGMENT_SIZE`. `segment/tests.rs` pins both placement contracts through
  `fresh_segment_installs_header_guard_in_page_0` and
  `fresh_segment_installs_tail_guard_in_alignment_slack`; backend telemetry and
  null/zero rejection are covered by the guard module tests. No arena-level
  guard-placement follow-on remains in this scope.
- [resolved-history] The original Sprint B implementation note described the
  backend seam as lacking an arena consumer. It is superseded by the resolved
  entry above: opt-in header/tail guard placement is now wired through fresh
  segment creation and covered by the named placement and telemetry tests.
