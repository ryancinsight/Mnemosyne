...
- [ ] [arch] status=in-progress owner=codex scope=`crates/mnemosyne-local/src/local_alloc/page.rs`, new `page/` leaves, callers, and matching PM entries; last-update=2026-07-26. Split the 546-line page module into named leaves for pointer provenance, page-local allocation/reclaim, branded intrusive-list primitives, and allocator list transitions. Acceptance: the manifest contains no implementation, each leaf owns one concern without duplicated logic or compatibility aliases, all existing callers retain value-semantic behavior, the touched crate remains warning-clean, and no performance gain is claimed.
