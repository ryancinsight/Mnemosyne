...
- [ ] [patch] status=in-progress owner=codex scope=`crates/mnemosyne-heap/src/heap.rs`, `crates/mnemosyne-heap/src/tests/`, and matching PM entries; last-update=2026-07-24. Compute the branded block's runtime layout before `drop_in_place` in `Heap::free`; the current post-drop `size_of_val` reference violates the initialized-value lifetime required by the unsafe operation. Acceptance: sized and unsized branded frees retain drop counts and release the allocation, the free path has no post-drop reference, and focused heap formatting, warning-denied Clippy, nextest, doctests, and rustdoc pass. No performance claim.
