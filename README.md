...
*   `mnemosyne-backend` is organized by backend responsibility:
    ...
    *   [`backends`](crates/mnemosyne-backend/src/backends/mod.rs) owns the per-OS / per-platform backend implementations: `UnixBackend`, `WindowsBackend`, `CudaUnifiedBackend`, `CudaDeviceBackend`, tier-keyed `CudaHbmBackend`/`CudaGddrBackend`, and `CudaHostPinnedBackend`. `DefaultBackend` selects the OS-conditional backing per the active platform at compile time.
*   Public re-exports at the crate root keep the canonical `mnemosyne_backend::CudaUnifiedBackend`, `CudaDeviceBackend`, `CudaHbmBackend`, `CudaGddrBackend`, `CudaHostPinnedBackend`, `MemoryBackendWrapper`, `DefaultBackend`, and `backend_memory_stats` paths; backend-specific helpers live under `backends` and are reached by qualified path.
*   Device-API resource ownership stays in the provider crate. WGPU mapped buffers are not exposed as a Mnemosyne `MemoryBackend` because WGPU's write-combining mapped ranges do not satisfy the general raw-pointer contract.
...
The CUDA unified-memory backend uses a three-state initialization gate for race-free dynamic symbol resolution and tracks managed allocations in a fixed-size registry. It returns null when CUDA is unavailable or registry capacity is exhausted; callers select a host backend explicitly when recovery is valid.
...
The current comparison table makes three gaps visible: huge-object deallocation latency is still much slower than MiMalloc, threaded scaling trails the top-tier designs in the small-allocation case, and the suite lacks sustained mixed-size churn/fragmentation workloads. These are documented in the gap summary at the top of `benchmarks/allocator_comparison.md` and are the highest-value additions for future benchmark work.
