...
*   `allocate_segment` installs a 4 KiB guard at `aligned_addr + SEGMENT_SIZE` on every fresh OS-backed segment, inside the alignment slack the arena already reserves. Forward OOB writes that walk past the last user page (Page 31) trap on the guard instead of corrupting an unrelated mapping.
