...
*   `WgpuStagingBackend` publishes one immutable process-global `&'static WgpuCallbacks` through a single atomic pointer. `WgpuCallbacks::new` is the unsafe lifetime/pairing boundary; `register_wgpu_callbacks` is safe, idempotent for the same static pair, and rejects a competing pair with `WgpuCallbackRegistrationError`. Allocation and deallocation therefore cannot observe different generations.
