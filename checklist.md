...
- [x] [patch] Replace `benchmark_summary` allocator-row `split('/').collect::<Vec<_>>()` plus `to_lowercase()` with borrowed `split_once` parsing and allocation-free `eq_ignore_ascii_case` classification; pin optional sub-benchmark parsing and exact classifier rejection with unit tests.
