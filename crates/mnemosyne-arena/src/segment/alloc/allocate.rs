//! Handing a segment out: from a pool if one is retained, from the OS
//! otherwise, then initialized and guarded.
//!
//! Returning one is the other direction and lives in [`super::release`],
//! which reaches `try_return_to_pool` here through the parent.

use super::super::alignment::checked_align_up;
use super::super::pool::HasSegmentPool;
#[cfg(feature = "segment-header-guards")]
use super::SEGMENT_HEADER_GUARD_SIZE;
use super::{SEGMENT_MAPPING_SIZE, SEGMENT_TAIL_GUARD_SIZE};
...
/// by the caller.
#[inline(always)]
pub(super) unsafe fn try_return_to_pool<B: HasSegmentPool>(segment: *mut Segment) -> bool {
    debug_assert!(
        !segment.is_null(),
